    ...
    return (
        ...
        >
            <div className="max-w-screen-xl mx-auto flex items-center justify-between h-full px-4">
                {/* Logo */}
                <Link href="/">
                    <img src="/chokoLogo2.png" className="h-16 object-contain" alt="Logo" />
                </Link>
                ...
                <nav className="hidden md:flex items-center space-x-6">
                    {navItems.map((item, index) => (
                        <Link key={index} href={item.path}>
                            <span
                                className={`cursor-pointer ${isScrolled ? "text-black" : "text-white"} hover:text-primary transition-colors duration-200`}
                            >
                                {item.name}
                            </span>
                        </Link>
